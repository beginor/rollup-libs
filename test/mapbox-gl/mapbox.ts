...
function addSky(map: mapboxgl.Map) {
    map.addLayer({
        'id': 'sky',
        'type': 'sky',
        'paint': {
...
function freeCamera(map: mapboxgl.Map): void {
    const animations = [
        {
            duration: 20000.0,
            animate: (phase: number) => {
                ...
                // lat/lng
                const position = lerpArr(start, end, phase) as mapboxgl.LngLatLike;
                const altitude = lerp(alt[0], alt[1], phase);
                const target: mapboxgl.LngLatLike = [138.73036, 35.36197];
        ...
        },
        {
            ...
            animate: (phase: number) => {
                ...
                const position = lerpArr(start, end, phase) as mapboxgl.LngLatLike;
                const altitude = lerp(alt[0], alt[1], phase);
                const target = lerpArr(target1, target2, phase) as mapboxgl.LngLatLike;
                updateCameraPosition(map, position, altitude, target);
            }
        },
        {
            ...
            animate: (phase: number) => {
                ...
                // interpolate both the camera position and target
                const position = lerpArr(start, end, easeInOutQuad(phase)) as mapboxgl.LngLatLike;
                const altitude = lerp(alt[0], alt[1], phase);
                const target = lerpArr(target1, target2, phase) as mapboxgl.LngLatLike;
...
async function addCities(map: mapboxgl.Map): Promise<void> {
    const res = await fetch('../../assets/cities.geojson');
    const points = await res.json() as FeatureCollection<Point>;
